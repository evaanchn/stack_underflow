...
GameInfoWindow::GameInfoWindow(int windowWidth, int windowHeight, const char* title) {
  window = new Fl_Window(windowWidth, windowHeight, title);
  this->window->color(FL_DARK_BLUE);  // 💡 Window background color
  ...
  int buttonsY = windowHeight - CONFIRM_BUTTONS_HEIGHT - 20; // 20px margin from bottom
  ...
  display = new Fl_Text_Display(displayX, displayY, displayWidth, displayHeight);
  ...
  // Cancel Button
  cancelButton = new Fl_Button(buttonsX + CONFIRM_BUTTONS_WIDTH + CONFIRM_BUTTONS_SPACING,
      buttonsY, CONFIRM_BUTTONS_WIDTH, CONFIRM_BUTTONS_HEIGHT, "Cancel");
